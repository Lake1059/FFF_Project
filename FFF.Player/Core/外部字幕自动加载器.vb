...
Public NotInheritable Class 外部字幕轨道
    ...
    ''' 才释放 SUP 原生解码器，避免原子换轨与正在生成的位图帧互相踩踏。
    ''' </summary>
    ...
    Private Sub 释放资源一次()
        If Interlocked.Exchange(已释放资源, 1) = 0 Then SUP生成器?.Dispose()
    End Sub
End Class
...
Public NotInheritable Class 外部字幕自动加载器
    ...
    ''' <summary>完整解析指定字幕；成功返回的轨道可原子替换当前轨道。</summary>
    Public Shared Function 加载字幕(字幕路径 As String,
        ...
        Select Case 格式
            ...
            Case 外部字幕格式.SUP
                ...
                Try
                    取消令牌.ThrowIfCancellationRequested()
                    Return New 外部字幕轨道(完整路径, 格式, Nothing, Nothing, 生成器)
                Catch
                    生成器.Dispose()
                    Throw
                End Try
