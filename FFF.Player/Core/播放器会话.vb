...
Public NotInheritable Class 播放器会话
    ...
    Public Sub New(配置 As 播放器配置)
        ...
        配置.验证()
        If 播放器原生接口.FFF3FP_GetApiVersion() <> 6UI Then Throw New InvalidOperationException("FFF.Native 的 3FP API 版本不兼容。")
        同步上下文 = 配置.事件同步上下文
        Dim 状态 = New 回调状态()
        ...
        Try
            ...
            Dim 原生配置 As New 原生播放器配置 With {
                .大小 = CUInt(Marshal.SizeOf(Of 原生播放器配置)()), .版本 = 6UI,
    ...
    Public ReadOnly Property 当前快照 As 播放器快照
        Get
            Dim 值 As New 原生播放器快照 With {.大小 = CUInt(Marshal.SizeOf(Of 原生播放器快照)()), .版本 = 6UI}
