...
常规滚动最大行数、行间距、边距、外描边、阴影、固定弹幕持续时间和相对缩放基准。滚动位置直接
使用连续媒体时钟和小数像素；目标帧率只定义呈现节奏及诊断帧序号。弹幕横向以完整客户区为边界，
纵向布局及缩放仍以有效视频高度为基准，因此左右黑边不会暴露突现或突消。跳转、倒退、窗口尺寸
变化或配置变化会清空活动项并从当前位置继续读取，不回溯恢复旧弹幕；连续播放只推进时间游标。
...
Seek 不回溯、DirectWrite 自然对称渲染、灰度抗锯齿、纯外描边及 45 度阴影：
