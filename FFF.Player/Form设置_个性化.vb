...
Public Class Form设置_个性化
    ...
    Friend Sub 初始化页面()
        ...
        Try
            If SP_UnLock Then
                HtmlColorLabel1.Text = "感谢您支持 FFF.Player Supporter Pack"
    ...
    Private Sub MB_前往购买_Click(sender As Object, e As EventArgs) Handles MB_前往购买.Click
        Process.Start(New ProcessStartInfo With {
            .FileName = "https://ifdian.net/item/ed2719d4841211f1a67352540025c377", .UseShellExecute = True})
    End Sub

    Private Sub MB_图标_Click(sender As Object, e As EventArgs) Handles MB_图标.Click
        If Not SP_UnLock Then Return
        Using dialog As New OpenFileDialog With {.Multiselect = False, .Filter = "支持的图片|*.png;*.jpg;*.jpeg;*.gif"}
    ...
    Private Sub MB_窗口边框颜色_Click(sender As Object, e As EventArgs) Handles MB_窗口边框颜色.Click
        ...
        Using dialog As New ModernColorDialog With {
            .SelectedColor = Form1.当前主窗体.ThisIsYourWindow1.BorderColor, .Icon = Form1.当前主窗体.Icon}
            Form1.当前主窗体.ThisIsYourWindow1.Attach(dialog)
            If dialog.ShowDialog(Form1.当前主窗体) <> DialogResult.OK Then Return
            保存颜色(dialog.SelectedColor, True)
        End Using
        设置.应用SP个性化设置()
    End Sub
    ...
    Private Sub MB_分层阴影颜色_Click(sender As Object, e As EventArgs) Handles MB_分层阴影颜色.Click
        If Not SP_UnLock Then Return
        Using dialog As New ModernColorDialog With {
            .SelectedColor = Form1.当前主窗体.ThisIsYourWindow1.LayerShadowColor, .Icon = Form1.当前主窗体.Icon}
            Form1.当前主窗体.ThisIsYourWindow1.Attach(dialog)
            If dialog.ShowDialog(Form1.当前主窗体) <> DialogResult.OK Then Return
            保存颜色(dialog.SelectedColor, False)
        End Using
        设置.应用SP个性化设置()
    End Sub

    Private Shared Sub 保存颜色(color As Color, border As Boolean)
        If border Then
            设置.实例对象.SP_窗口边框颜色_A = color.A : 设置.实例对象.SP_窗口边框颜色_R = color.R
            设置.实例对象.SP_窗口边框颜色_G = color.G : 设置.实例对象.SP_窗口边框颜色_B = color.B
        Else
            设置.实例对象.SP_分层阴影颜色_A = color.A : 设置.实例对象.SP_分层阴影颜色_R = color.R
            设置.实例对象.SP_分层阴影颜色_G = color.G : 设置.实例对象.SP_分层阴影颜色_B = color.B
        End If
    End Sub
    ...
    Private Sub MB_选择背景图_Click(sender As Object, e As EventArgs) Handles MB_选择背景图.Click
        If Not SP_UnLock Then Return
        Using dialog As New OpenFileDialog With {.Multiselect = False, .Filter = "支持的图片|*.png;*.jpg;*.jpeg"}
            If dialog.ShowDialog(Form1.当前主窗体) <> DialogResult.OK Then Return
            Try
    ...
    Private Sub 更新玻璃附属控件状态()
        Dim enabled = SP_UnLock AndAlso MCB_毛玻璃模式.SelectedIndex > 0
