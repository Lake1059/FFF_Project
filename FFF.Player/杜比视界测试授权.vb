...
Friend NotInheritable Class 杜比视界测试授权
    ...
    Friend Shared Sub 初始化(owner As IWin32Window)
        ...
        Dim code = ExInputBox(owner, $"票据有效期为一个月{vbCrLf}如需刷新票据时间请删除票据文件并重新解锁{vbCrLf}{vbCrLf}此 DLL 仅限开发群内部测试使用！{vbCrLf}任何向外传播、公开使用、任何商业等行为导致违反杜比视界版权许可产生的纠纷均由使用者承担，与开发者没有任何关系！", "Dolby Vision 技术测试防传播验证")
