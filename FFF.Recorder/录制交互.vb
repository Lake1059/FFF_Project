...
Public Module 录制交互
    ...
    Public Sub 开始录制()
        ...
        Try
            ...
            Dim HDR输出 = 设置.实例对象.色彩模式 = 1
            Dim HDR标称峰值 = CUInt(Math.Max(设置.实例对象.HDR峰值, If(HDR输出, 设置.实例对象.SDR亮度, 1)))
            ...
            视频配置.设置色彩模式(HDR输出, 设置.实例对象.SDR亮度, HDR标称峰值)
    ...
    Private Sub 写入录制配置日志(源 As 视频源条目, 音频 As 音频源条目,
                             ...
                             输出文件 As String)
        Dim 捕获模式 = If(源.捕获模式 = 0, "显示器", If(源.捕获模式 = 2, "窗口客户区", "窗口带标题栏"))
        ...
        写日志($"色彩处理：SDR白电平={设置.实例对象.SDR亮度} nit，HDR标称峰值={配置.HDR标称峰值} nit，色彩范围={配置.色彩范围}", False)
