...
Public NotInheritable Class 处理后视频帧
    ...
    Friend Sub New(自有纹理 As ID3D11Texture2D, 时间戳 As Long, 回收操作 As Action(Of ID3D11Texture2D))
        纹理 = 自有纹理
        QPC时间戳 = 时间戳
        回收纹理 = 回收操作
    End Sub
...
Public NotInheritable Class 视频处理器
    ...
    Private Structure 缩放常量
        Public 源宽度 As UInteger
        Public 源高度 As UInteger
        ...
        Public 保留一 As Single
        Public 保留二 As Single
    End Structure
            ...
            图形.执行图形命令(
                Sub()
                    Using 输入视图 = 图形.设备.CreateShaderResourceView(源纹理, Nothing)
                        Using 输出视图 = 图形.设备.CreateUnorderedAccessView(中间纹理, Nothing)
                        ...
                        End Using
                    End Using
                    Using 中间视图 = 图形.设备.CreateShaderResourceView(中间纹理, Nothing)
                        Using 最终视图 = 图形.设备.CreateRenderTargetView(输出纹理, Nothing)
                ...
                End Sub)
            中间纹理池.归还(中间纹理)
            Return New 处理后视频帧(输出纹理, 时间戳, AddressOf 输出纹理池.归还)
        Catch
            中间纹理池.归还(中间纹理)
            输出纹理池.归还(输出纹理)
            Throw
    ...
    Private Shared Function 读取着色器(资源名称 As String) As Byte()
