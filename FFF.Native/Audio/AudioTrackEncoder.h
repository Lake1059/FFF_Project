...
public:
    ...
    ~AudioTrackEncoder();
    // 在给定容器中建立 48 kHz stereo AAC stream；容器与 packetWriter 的生命周期由 VideoMuxer 保证。
    FFFResult Initialize(AVFormatContext* formatContext,
        std::function<FFFResult(const AVPacket*)> packetWriter,
        const std::string& title, float gain) noexcept;
    ...
    // 返回最近一次格式、重采样、AAC 或 packetWriter 错误的值副本。
    std::string LastError() const;
    // 返回最近音频包目标位置与 AAC FIFO 末端之间的样本误差，正值表示需要追赶。
...
private:
    // 首包时建立输入格式到编码器采样格式的 SwrContext；运行中格式改变会拒绝继续。
    ...
    // 向 AAC FIFO 追加输出格式静音，用于真实时间线缺口而非暂停区间。
    ...
    // 持续接收 AAC packet、换算时间基并转交异步写入回调，直到 EAGAIN 或 EOF。
