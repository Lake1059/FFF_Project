...
// 在尚未写 Matroska header 的 AVFormatContext 中创建 48 kHz 双声道 AAC 轨道。formatContext
// 与 packetWriter 均由 VideoMuxer 持有并覆盖本对象生命周期；title 会复制进 stream metadata。
FFFResult AudioTrackEncoder::Initialize(AVFormatContext* formatContext,
    std::function<FFFResult(const AVPacket*)> packetWriter,
    const std::string& title, const float gain) noexcept {
    ...
    if (codec == nullptr) {
        lastError_ = "The FFmpeg build does not contain an AAC encoder.";
    ...
    codecContext_->bit_rate = 192'000;
    codecContext_->time_base = { 1, 48'000 };
    ...
    if (result < 0) {
        SetFfmpegError("avcodec_open2(AAC)", result);
        ReleaseResources();
        return FFFResult::FfmpegFailure;
    }
    ...
    av_dict_set(&stream_->metadata, "title", title.c_str(), 0);
...
FFFResult AudioTrackEncoder::Encode(const std::uint8_t* data, const std::uint32_t frameCount,
    ...
    if (timelineError > 4'800) {
        const auto silenceResult = AppendSilence(static_cast<std::int32_t>(std::min<std::int64_t>(timelineError, 48'000)));
        ...
        swr_set_compensation(resampler_, compensation, 48'000);
        compensationPpm_.store(static_cast<std::int32_t>(compensation * 1'000'000LL / 48'000LL));
    }
    ...
    auto result = av_samples_alloc_array_and_samples(&converted, nullptr,
    ...
    if ((flags & AUDCLNT_BUFFERFLAGS_SILENT) != 0 || data == nullptr) {
        ...
        inputPointer = silence.data();
    }
    const std::uint8_t* inputPlanes[] = { inputPointer };
    const auto convertedSamples = swr_convert(resampler_, converted, outputCapacity,
    ...
    ApplyGain(converted, convertedSamples);
    if (av_audio_fifo_realloc(fifo_, av_audio_fifo_size(fifo_) + convertedSamples) < 0 ||
        av_audio_fifo_write(fifo_, reinterpret_cast<void**>(converted), convertedSamples) < convertedSamples) {
...
FFFResult AudioTrackEncoder::EnsureResampler(const WasapiSampleFormat& inputFormat) noexcept {
    if (inputFormatInitialized_) {
        if (std::memcmp(&inputFormat_, &inputFormat, sizeof(inputFormat)) != 0) {
            lastError_ = "The WASAPI endpoint mix format changed during recording.";
...
FFFResult AudioTrackEncoder::EncodeAvailableFrames(const bool flushPartialFrame) noexcept {
    const auto frameSize = codecContext_->frame_size > 0 ? codecContext_->frame_size : 1024;
...
FFFResult AudioTrackEncoder::DrainPackets() noexcept {
    ...
    while (true) {
        ...
        av_packet_rescale_ts(packet, codecContext_->time_base, stream_->time_base);
        packet->stream_index = stream_->index;
        const auto writeResult = packetWriter_(packet);
