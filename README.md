...
- 三挡 HDR 模式，适配不同的屏幕和场景
  ...
  | 真实 HDR 高亮 | 按源 HDR 规格映射到显示器实际亮度范围，需要 HDR 屏幕 |

  HDR 目标峰值默认从当前显示器的 Advanced Color 能力读取；配置值为 `0` 时自动适配，正值用于覆盖显示器报告值。播放器识别 HDR10、HDR10+、HLG、HDR Vivid 以及 Dolby Vision P5/P7/P8.1/P8.4。开源版本不宣称 Dolby Vision 解码或输出授权：Dolby 源统一转换为 Rec.2020/PQ 的 HDR10 兼容输出，P7 FEL 的增强层残差会被忽略。该回退不会伪造 HDR10+ 的 ST 2094-40 动态元数据。
